...
class Alg():
    ...
    def _alg1(self):
        ...
        def generate_combinations(index, current_combination):
            self._work[0] += 1  # Increment work
            if index >= n:
                # We have considered all indices, now process the combination
    ...
    def _alg2(self):
        ...
        if self._show:
            print('----------------------------------------------')
            print('Algorithm - 2')
            print("Complexity Time: O(n) Space:O(1)")
            print('----------------------------------------------')
